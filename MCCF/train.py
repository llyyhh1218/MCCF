...
from train_utils import (train_one_epoch, evaluate, init_distributed_mode,
                         save_on_master, mkdir, create_lr_scheduler)
from utils.PCLT_dataset import prepare_PETCT_dataset
import torch.distributed as dist

# 使用新的 VSSM_Fusion 架构（替代旧的 EncoderDecoder）
from models import create_vssm_tiny, VSSM_Fusion_Complete
...
# VSSM_Fusion 配置（使用新的架构）
C.model_type = 'tiny'  # tiny / small / base
C.num_classes = 1       # 分割类别数
C.in_chans = 3          # 输入通道数（3通道RGB，与evaluate函数兼容）
C.image_height = 512
C.image_width = 512
...
def create_model():
    # 使用新的 VSSM_Fusion_Complete 模型
    model = create_vssm_tiny(
        num_classes=C.num_classes,
        in_chans=C.in_chans,
        img_size=[C.image_height, C.image_width]
    )

    parameter = params_count(model)
    print(f'Model: VSSM_Fusion (type={C.model_type})')
    print(f'Parameters: {parameter / 1e6:.2f}M')
    print('Fusion Pipeline:')
    print('  Stage 1: ChannelRectifyModule (CRM)')
    print('  Stage 2: ConMB_SS2D (Feature Processor)')
    print('  Stage 3: VSSBlock_Cross_new (Cross-Modal Fusion)')
    return model
